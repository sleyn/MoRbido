...
#' @param C0 - Concentration at the beginning
#' If FALSE - files for the tables of OD and Concetration changes will be created separately (*_OD.txt, *_Conc.txt)

#' @export

write.m.tables <- function(tables, united = F, col = F, C0 = 0){
  #united == T - create united table of time, OD and concentration
  if( col == F ){
    for(i in 1:length(tables)){
      tV <- tables[i][[1]][1]$OD
      tC <- tables[i][[1]][2]$Conc
      tC <- rbind(tC, data.frame(time = tail(tV$time, 1) + 1,Concentration = tail(tC$Concentration, 1),Bottle = tail(tC$Bottle, 1)))
      tube <- names(tables)[i]
      ...
      if(united == T){
        ...
        for(j in 1:dim(tC)[1]){
          tU$concentration[tU$time >= tC$time[j] & tU$time < tC$time[j+1]] = tC$Concentration[j]
        }
